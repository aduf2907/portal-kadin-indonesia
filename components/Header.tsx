...
import { BellIcon, CheckCircleIcon, ExclamationTriangleIcon, InformationCircleIcon } from './icons';
...
const NotificationBell: React.FC<{ 
    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setIsOpen(!isOpen)} className="relative p-2 text-slate-600 hover:text-sky-700 focus:outline-none" aria-label={`Notifications (${unreadCount} unread)`}>
                <BellIcon className="w-6 h-6" />
                {unreadCount > 0 && (
                    <span className="absolute top-1 right-1 block h-3 w-3 rounded-full bg-red-500 border-2 border-white" aria-hidden="true"></span>
                )}
            </button>
            {isOpen && (
                <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-lg shadow-xl border border-slate-200 overflow-hidden z-50">
                    <div className="p-3 flex justify-between items-center border-b">
                        ...
                        {unreadCount > 0 && <button onClick={onMarkAllAsRead} className="text-sm text-sky-600 hover:underline">Mark all as read</button>}
                    </div>
                    <div className="max-h-96 overflow-y-auto">
                        {notifications.length > 0 ? (
                             [...notifications].reverse().map(notif => (
                                <div key={notif.id} onClick={() => onMarkAsRead(notif.id)} className={`flex items-start gap-3 p-3 border-b cursor-pointer hover:bg-slate-50 ${!notif.read ? 'bg-sky-50' : ''}`}>
                                    <div className="flex-shrink-0 mt-1">
                                      <NotificationIcon type={notif.type} />
                                    </div>
                                    <div className="flex-grow">
                                        <p className={`text-sm ${!notif.read ? 'font-semibold text-slate-800' : 'text-slate-600'}`}>{notif.message}</p>
                                        <p className="text-xs text-slate-400 mt-1">{timeSince(notif.timestamp)}</p>
                                    </div>
                                    {!notif.read && <div className="w-2 h-2 bg-sky-500 rounded-full mt-2 flex-shrink-0" aria-label="Unread"></div>}
                                </div>
                            ))
                        ) : (
...
const Header: React.FC<HeaderProps> = ({ isAuthenticated, onLogout, navigateTo, currentPage, notifications, markNotificationAsRead, markAllNotificationsAsRead }) => {
        ...
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="font-bold text-lg text-amber-600 leading-tight">KADIN</span>
              <span className="text-sm text-slate-600 leading-tight">Indonesia</span>
            </div>
          </div>
            ...
            {isAuthenticated ? (
              <>
                <div className="hidden sm:flex items-center gap-4">
                  <NavLink page="dashboard" currentPage={currentPage} navigateTo={navigateTo}>Dashboard</NavLink>
                  <NavLink page="berita" currentPage={currentPage} navigateTo={navigateTo}>Berita</NavLink>
                  <NavLink page="directory" currentPage={currentPage} navigateTo={navigateTo}>Direktori</NavLink>
                  <NavLink page="forum" currentPage={currentPage} navigateTo={navigateTo}>Forum</NavLink>
                  <NavLink page="kadin360" currentPage={currentPage} navigateTo={navigateTo}>Kadin 360</NavLink>
                </div>
                <NotificationBell 
      ...
      {showLogoutConfirm && (
        ...
        >
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm transform transition-all animate-fade-in-scale">
            <h3 id="logout-dialog-title" className="text-xl font-bold text-slate-900">Confirm Logout</h3>
